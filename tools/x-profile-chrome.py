#!/usr/bin/env python3
"""
X Profile Setup - Uses existing Chrome profile with login session
"""
...
async def main():
    print("=" * 60)
    print("X PROFILE SETUP (Using Chrome Profile)")
    print("=" * 60)
    
    async with async_playwright() as p:
        # Use existing Chrome profile
        print("[1/8] Launching Chrome with your profile...")
        ...
        try:
            # Check login
            print("[2/8] Checking login status...")
            await page.goto("https://x.com/home")
            await page.wait_for_load_state("networkidle")
            await asyncio.sleep(3)
            
            if "login" in page.url.lower() or await page.locator('text="Sign in"').count() > 0:
                print("[ERROR] Not logged in! Please log into X in Chrome first.")
                await context.close()
                return
